...
export default function PostPage() {
  ...
  sortedBlocks?.map((block: Block) => {
    if (block.type === "prediction" && prediction.length === 0) {
      api.get(`/predictions/matches`)
        .then(res => {
          setPrediction(res.data);
          setContentLoading(false); // 완료 표시, prediction에 대해서만 사용 중이나 확장이 필요
        })
  ...
  return (
    <article className="max-w-5xl mx-auto px-4 py-6 rounded-xl bg-base-200 space-y-6">
      <div className="flex">
        ...
        <button className="btn btn-primary p-2 bg-base-100">뒤로</button>
      </div>
      ...
      <div className="space-y-4 p-4 bg-base-100 rounded-lg text-base-content">
        {sortedBlocks.map((block : any) => { //TextBlock으로 가정, 추후 확장
          ...
          if(block.type === "prediction" && !contentLoading) {
